...
| `nl_llm` | 算力与网关层，防雪崩 | `gateway.rs` (编排层)、`prompt_ast.rs` (Prompt AST)、`token_bucket.rs` (流量控制)、`fallback.rs` (降级路由)、`provider/black_magic_proxy.rs` (黑魔法代理) |
...
`nl_llm` 实现模型无关的防腐层设计，核心模块：

```
nl_llm/src/
├── gateway.rs           # LLM Gateway 编排层
├── prompt_ast.rs        # Prompt AST 结构 (模型无关)
├── prompt.rs            # Prompt 构建器
├── token_bucket.rs      # 全局令牌桶限流
├── fallback.rs          # 降级路由
└── provider/
    ├── anthropic.rs     # Anthropic 适配器 (XML 格式)
    ├── openai.rs        # OpenAI 适配器 (JSON 格式)
    ├── ollama.rs        # Ollama 适配器 (ChatML 格式)
    ├── cli_proxy.rs     # CLI PTY 代理
    └── black_magic_proxy.rs  # 黑魔法代理聚合层
```
...
| 项目 | 代理形态 | 认证方式 |
|------|----------|----------|
| CLIProxyAPI | `Api` + `Cli` | OAuth、API Key、多账户轮询 |
| new-api | `Api` + `Auth` | API Key��OAuth (GitHub/Discord/Telegram) |
| cc-switch | `Api` + `WebSocket` | 本地配置、环境变量 |
| Claude Code Router | `Api` + `WebSocket` | x-api-key 头 |
