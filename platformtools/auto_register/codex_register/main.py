...
def _probe_loop() -> None:
    ...
    while True:
        try:
            ...
            # 触发续杯条件（pool=10: >=8 invalid_like）
            pool_size = min(TARGET_POOL_SIZE, len(reports_for_probe))
            need_topup = (pool_size < TARGET_POOL_SIZE) or (invalid_like >= TRIGGER_INVALID_THRESHOLD and pool_size >= TARGET_POOL_SIZE)
...
def register(driver, proxy=None) -> tuple[str, str]:
    ...
    try:
        ...
        try:
            _dbg("ui", "after ENTER on continue step", driver=driver)
            _dump_page_body(driver=driver, kind="after_continue_enter", message="after ENTER on continue step")
        except Exception:
            pass
