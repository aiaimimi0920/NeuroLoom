...
json_normalize_wham_status() {
  # 与 单次续杯.bat / 单次续杯.sh 中的同名函数逻辑保持一致
  local raw_status="$1"
  local body_file="$2"

  # 非 3 位数字或 000 → 直接返回 000
  if ! [[ "$raw_status" =~ ^[0-9]{3}$ ]] || [[ "$raw_status" == "000" ]]; then
    echo "000"
    return 0
  fi
  # 已经是 401/429 → 直接返回
  if [[ "$raw_status" == "401" || "$raw_status" == "429" ]]; then
    echo "$raw_status"
    return 0
  fi
  # 非 2xx → 直接返回原状态码
  if [[ "${raw_status:0:1}" != "2" ]]; then
    echo "$raw_status"
    return 0
  fi
  # 2xx → 用 python3 精确检查 body 中的限流/禁用字段
  if [[ -f "$body_file" ]]; then
    local q
    q="$(python3 - "$body_file" <<'PY' 2>/dev/null || echo "0"
import json, sys
q = 0
try:
    ...
    if rl is not None:
        ...
        up = pw.get('used_percent')
        if up is not None and float(up) >= 100:
            q = 1
...
OUT_DIR="$ROOT_DIR/out/清理-401-$(date +%Y%m%d-%H%M%S)"
...
if [[ "$APPLY" == "1" && "$deleted" -gt 0 ]]; then
  ...
  if [[ "$missing" -gt 0 ]]; then
    ...
    OUT_REFILL="$ROOT_DIR/out/清理后补齐结果.jsonl"
