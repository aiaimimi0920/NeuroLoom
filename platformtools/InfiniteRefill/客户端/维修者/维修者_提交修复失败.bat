...
REM 维修者：提交修复失败（POST /v1/repairs/submit-failed）
REM
REM 用法：
REM   维修者_提交修复失败.bat 服务器地址 维修者密钥 artwork_id [note]
REM
set "服务器地址=%~1"
set "维修者密钥=%~2"
set "ARTWORK_ID=%~3"
...
powershell -NoProfile -Command "$body=@{ artwork_id='%ARTWORK_ID%'; note='%NOTE%' } | ConvertTo-Json -Depth 10; Write-Output $body" ^
  | curl -sS -X POST "%服务器地址%/v1/repairs/submit-failed" ^
...
echo 示例：%~nx0 http://127.0.0.1:8788 k_xxx acc_demo_1 "无法修复：缺字段"
echo.
exit /b 1
