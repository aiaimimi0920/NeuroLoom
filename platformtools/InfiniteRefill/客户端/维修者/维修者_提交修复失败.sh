...
# 维修者：提交修复失败（POST /v1/repairs/submit-failed）
# 目标：尽量不依赖额外工具（macOS 优先用系统自带 osascript/JXA 生成请求体）。
#
# 用法：
#   bash 维修者_提交修复失败.sh 服务器地址 维修者密钥 artwork_id [note]
...
if [[ -z "$SERVER_URL" || -z "$REPAIRER_KEY" || -z "$ARTWORK_ID" ]]; then
  ...
  echo "示例：$0 http://127.0.0.1:8788 k_xxx acc_demo_1 '无法修复'" >&2
  exit 1
fi
...
make_body \
  | curl -sS -X POST "$SERVER_URL/v1/repairs/submit-failed" \
