...
async fn main() -> anyhow::Result<()> {
    ...
    let client = LlmClient::from_preset("kling")
        ...
        .auth(nl_llm::auth::providers::kling::KlingAuth::new(token))
        .build();

    // 2. 构造原语请求
    let mut req = PrimitiveRequest::new("kling-v1"); // 可以是 kling-v1, kling-v1-6, kling-v2-master
    req.messages.push(nl_llm::primitive::PrimitiveMessage {
        role: nl_llm::primitive::Role::User,
        content: vec![nl_llm::primitive::PrimitiveContent::Text {
