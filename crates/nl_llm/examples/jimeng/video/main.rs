...
use nl_llm::primitive::{PrimitiveContent, PrimitiveMessage, PrimitiveRequest};
...
async fn main() -> anyhow::Result<()> {
    ...
    request.messages.push(PrimitiveMessage {
        role: nl_llm::primitive::Role::User,
    ...
    loop {
        ...
        match client.fetch_video_task(&task_id).await {
            Ok(status) => match status.state {
                nl_llm::provider::extension::VideoTaskState::Submitted => {
                    println!("...任务已提交并排队中 (Task ID: {})", task_id);
                }
                nl_llm::provider::extension::VideoTaskState::Processing => {
                    println!("...任务处理中 (Task ID: {})", task_id);
                }
                nl_llm::provider::extension::VideoTaskState::Succeed => {
                    println!("🎉 任务生成成功!");
                ...
                nl_llm::provider::extension::VideoTaskState::Failed => {
                    println!("❌ 任务生成失败!");
                    if let Some(msg) = status.message {
                        println!("错误信息: {}", msg);
                    }
