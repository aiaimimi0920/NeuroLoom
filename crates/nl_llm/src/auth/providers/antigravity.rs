...
impl AntigravityOAuth {
    ...
    async fn resolve_project_id_extra(
        ...
        match self.fetch_project_id(access_token).await {
            ...
            Ok(None) => {
                self.log(&format!(
                    "fetch_project_id returned None during {}",
                    context
                ));
            }
            Err(e) => {
    ...
    async fn fetch_project_id(&self, access_token: &str) -> anyhow::Result<Option<String>> {
        ...
        if !res.status().is_success() {
            ...
            let body = res.text().await.unwrap_or_default();
            self.log(&format!(
                "loadCodeAssist failed with status {}: {}",
                status, body
            ));
            return Ok(None);
        }
        ...
        if !project_id.is_empty() {
            self.log(&format!(
                "Got project_id from loadCodeAssist: {}",
                project_id
            ));
        ...
        self.log(&format!(
            "Attempting onboard_user with tier_id: {}",
            tier_id
        ));
    ...
    async fn onboard_user(
        ...
        for attempt in 0..3 {
            ...
            if res.status().is_success() {
                ...
                self.log(&format!(
                    "onboardUser attempt {} failed with status {}",
                    attempt + 1,
                    res.status()
                ));
            }
...
impl Default for DynamicOAuthConfig {
    fn default() -> Self {
        Self {
            ...
            scopes: ANTIGRAVITY_OAUTH_CONFIG
                .scopes
                .iter()
                .map(|s| s.to_string())
                .collect(),
