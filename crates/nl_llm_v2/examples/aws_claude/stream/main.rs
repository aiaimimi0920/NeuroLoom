//! AWS Claude (Amazon Bedrock) 流式输出测试
//!
//! ## 特性演示
//!
//! - 流式输出
...
//!
//! 运行方式:
//!   方式1: cargo run -p nl_llm_v2 --example aws_claude_stream -- <api_key> [prompt]
//!   方式2: 设置 AWS_BEDROCK_API_KEY 环境变量
...
async fn main() -> anyhow::Result<()> {
    let api_key = std::env::var("AWS_BEDROCK_API_KEY")
        ...
        .unwrap_or_else(|_| {
            eprintln!("用法: aws_claude_stream <API_KEY> [prompt]");
            eprintln!("或设置 AWS_BEDROCK_API_KEY 环境变量");
            std::process::exit(1);
        });
    ...
    // ========== 创建带并发控制的客户端 ==========
    println!("========================================");
    println!("  AWS Claude 流式输出 + 并发控制演示");
    println!("========================================\n");
    ...
    let client = LlmClient::from_preset("aws_claude")
        ...
        // 启用并发控制（使用自定义配置）
        .with_concurrency_config(ConcurrencyConfig::new(10)  // AWS Bedrock 建议并发数
    ...
    // ========== 使用 "sonnet" 别名发送流式请求 ==========
    let model_alias = "sonnet";  // 解析为 Claude Sonnet 4.6
