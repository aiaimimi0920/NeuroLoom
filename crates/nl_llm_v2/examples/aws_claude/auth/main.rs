//! AWS Claude (Amazon Bedrock) 认证验证测试
//!
//! ## 特性演示
//!
//! - API Key 认证验证
...
//!
//! 运行方式:
//!   方式1: cargo run -p nl_llm_v2 --example aws_claude_auth -- <api_key>
...
async fn main() -> anyhow::Result<()> {
    let api_key = std::env::var("AWS_BEDROCK_API_KEY")
        ...
        .unwrap_or_else(|_| {
            eprintln!("用法: aws_claude_auth <API_KEY>");
            eprintln!("或设置 AWS_BEDROCK_API_KEY 环境变量");
            std::process::exit(1);
        });

    println!("========================================");
    println!("  AWS Claude (Bedrock) 认证验证");
    println!("========================================\n");
    ...
    match client.complete(&req).await {
        ...
        Err(e) => {
            ...
            println!("  2. 网络连接问题");
            println!("  3. AWS Bedrock 服务暂时不可用");
            println!("  4. 区域配置不正确（默认: us-east-1）");
            println!("\n获取 API Key: AWS Bedrock 控制台");
            std::process::exit(1);
        }
    }
