...
//!
//! 运行方式:
//!   方式1: cargo run -p nl_llm_v2 --example cohere_stream -- <api_key> [prompt]
...
async fn main() -> anyhow::Result<()> {
    let api_key = std::env::var("COHERE_API_KEY")
        ...
        .unwrap_or_else(|_| {
            eprintln!("用法: cohere_stream <API_KEY> [prompt]");
    ...
    println!("========================================");
    println!("  Cohere 流式输出 + 并发控制演示");
    println!("========================================\n");

    let client = LlmClient::from_preset("cohere")
        .expect("Preset should exist")
        .with_api_key(&api_key)
        .with_concurrency_config(ConcurrencyConfig::new(20)
