...
//!
//! 运行方式:
//!   方式1: cargo run -p nl_llm_v2 --example deepseek_stream -- <api_key> [prompt]
...
async fn main() -> anyhow::Result<()> {
    let api_key = std::env::var("DEEPSEEK_API_KEY")
        ...
        .unwrap_or_else(|_| {
            eprintln!("用法: deepseek_stream <API_KEY> [prompt]");
    ...
    let client = LlmClient::from_preset("deepseek")
        ...
        // 启用并发控制（使用自定义配置）
        .with_concurrency_config(ConcurrencyConfig::new(15)  // DeepSeek 建议并发数
    ...
    // ========== 使用 "ds" 别名发送流式请求 ==========
    let model_alias = "ds";  // 解析为 deepseek-chat
