...
//!
//! 运行方式:
//!   方式1: cargo run -p nl_llm_v2 --example kat_coder_auth -- <api_key>
//!   方式2: 设置 KAT_CODER_API_KEY 后运行
...
async fn main() -> anyhow::Result<()> {
    ...
    let is_dummy_key = api_key == "invalid_dummy_key_for_testing" || api_key == "blank";

    println!("========================================");
    println!("  KAT-Coder (StreamLake) 认证验证");
    println!("========================================\n");
    ...
    match client.complete(&req).await {
        ...
        Err(e) => {
            ...
            println!("  2. 网络无法连接到 vanchin.streamlake.ai");
            println!("  3. Endpoint ID 与 API Key 不匹配");
            println!("\n您可以前往获取 API Key: https://console.streamlake.ai/console/api-key");
