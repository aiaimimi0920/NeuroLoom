//! 讯飞星火 鉴权诊断
//!
//! 运行方式: cargo run -p nl_llm_v2 --example spark_auth -- <api_key:api_secret>
...
async fn main() -> anyhow::Result<()> {
    ...
    println!("Chat 能力: {}", if client.has_capability("ultra", Capability::CHAT) { "✅" } else { "❌" });
    println!("Tools 能力: {}", if client.has_capability("ultra", Capability::TOOLS) { "✅" } else { "❌" });
    ...
    match client.complete(&req).await {
        ...
        Err(e) => {
            eprintln!("❌ 鉴权失败: {}", e);
            eprintln!("\n排查建议:");
            eprintln!("  1. 确认 API Key 格式为 APIKey:APISecret（中间用冒号分隔）");
