//! Cubence 工具调用示例
//!
//! 演示如何使用 Cubence 进行工具调用 (Function Calling)
//!
//! 运行方式:
//!   - 设置环境变量: set CUBENCE_API_KEY=your-key
//!   - 运行: cargo run -p nl_llm_v2 --example cubence_tools
...
async fn main() -> anyhow::Result<()> {
    let api_key = std::env::var("CUBENCE_API_KEY")
        ...
        .unwrap_or_else(|_| {
            eprintln!("用法: cubence_tools <API_KEY>");
    ...
    println!("========================================");
    println!("  Cubence 工具调用");
    println!("========================================\n");
    println!("模型: gpt-4o (支持工具调用)");
    println!("用户: {}\n", prompt);
