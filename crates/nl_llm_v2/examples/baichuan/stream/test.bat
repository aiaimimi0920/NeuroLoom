...
if "%API_KEY%"=="" (
    if "%1"=="" (
        ...
        echo 请在 nl_llm_v2 目录下创建 baichuan_key.txt 存放您的密钥。
        echo.
...
set PROMPT=请写一首关于宇宙边缘的科幻短诗，要求格式优美。
if not "%2"=="" set PROMPT=%2

echo [1/2] 编译流式对话示例...
echo.
cargo build --example baichuan_stream
if %errorlevel% neq 0 (
    echo [错误] 编译失败！
    pause
    exit /b %errorlevel%
...
echo.
echo [2/2] 执行 Baichuan 流式对话示例...
echo =======================================================
cargo run --example baichuan_stream -- "%API_KEY%" "%PROMPT%"
echo =======================================================
