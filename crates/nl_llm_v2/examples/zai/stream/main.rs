...
//!
//! 运行方式:
//!   cargo run -p nl_llm_v2 --example zai_stream -- <api_key> [prompt]
//! 或设置 ZAI_API_KEY 环境变量后:
//!   cargo run -p nl_llm_v2 --example zai_stream
...
async fn main() -> anyhow::Result<()> {
    let api_key = std::env::var("ZAI_API_KEY")
        ...
        .unwrap_or_else(|_| {
            eprintln!("用法: zai_stream <API_KEY> [prompt]");
    ...
    // ========== 使用别名发送流式请求 ==========
    let model_alias = "flash";  // 解析为 glm-4-flash
