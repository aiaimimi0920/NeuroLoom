...
async fn main() -> anyhow::Result<()> {
    let api_key = std::env::var("SILICONFLOW_API_KEY")
        ...
        .unwrap_or_else(|_| {
            eprintln!("用法: siliconflow_auth <API_KEY>");
            eprintln!("或设置 SILICONFLOW_API_KEY 环境变量");
            std::process::exit(1);
        });

    println!("========================================");
    println!("  SiliconFlow (硅基流动) 认证验证");
    println!("========================================\n");
    println!("网关: https://api.siliconflow.cn/v1");
    ...
    println!("\n尝试基础通信 (Pro/moonshotai/Kimi-K2.5)...");
